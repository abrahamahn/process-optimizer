...
fn window() {
    ...
    unsafe {
        ...
        class.hInstance = instance; class.lpszClassName = name.as_ptr(); class.lpfnWndProc = Some(proc);
        RegisterClassW(&class);
        let h = CreateWindowExW(0, name.as_ptr(), wide("Optimizer test fixture").as_ptr(), WS_OVERLAPPEDWINDOW, 0, 0, 160, 120, null_mut(), null_mut(), instance, null());
        if h.is_null() { std::process::exit(1); }
        let mut msg = zeroed();
