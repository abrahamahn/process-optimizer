...
use crate::{
    gpu::{parse_instance, valid_percent},
    model::*,
    policy,
};
...
fn values(counter: isize, format: u32) -> AppResult<Vec<(String, f64)>> {
    for _ in 0..3 {
        ...
        if status != MORE_DATA {
            return Err(format!("Counter data unavailable: {status:#x}"));
        }
        if bytes == 0 || bytes > 64 * 1024 * 1024 {
            return Err("Invalid PDH buffer size.".into());
        }
        ...
        if status != 0 {
            return Err(format!("Counter sample unavailable: {status:#x}"));
        }
        if bytes as usize > capacity || count as usize > (bytes as usize) / size_of::<CounterItem>()
        {
            return Err("Invalid PDH item count.".into());
        }
        let start = data.as_ptr() as usize;
        let end = start + bytes as usize;
        let items = unsafe {
            std::slice::from_raw_parts(data.as_ptr().cast::<CounterItem>(), count as usize)
        };
        ...
        for item in items {
            ...
            let Some(n) = units.iter().position(|v| *v == 0) else {
                continue;
            };
            let number = unsafe {
                if format == LARGE {
                    item.value.number.large_value as f64
                } else {
                    item.value.number.double_value
                }
            };
            if number.is_finite() && number >= 0.0 {
                result.push((String::from_utf16_lossy(&units[..n]), number));
            }
        }
        return Ok(result);
    }
...
pub fn snapshot() -> AppResult<Snapshot> {
    ...
    if status != 0 {
        return Ok(Snapshot {
            processes: before,
            gpu_status: format!(
                "GPU counters unavailable ({status:#x}); no GPU usage is assumed to be zero."
            ),
        });
    }
    ...
    for path in paths {
        ...
        if status == 0 {
            ...
            handles.push(None);
            notices.push(format!("{path}: {status:#x}"));
        }
    }
    ...
    if first != 0 || second != 0 {
        return Ok(Snapshot {
            processes: process::enumerate().map_err(|e| e.message)?,
            gpu_status: format!(
                "GPU collection failed ({first:#x}/{second:#x}); all values are unknown."
            ),
        });
    }
    ...
    for (kind, counter) in handles.into_iter().enumerate() {
        ...
        for (name, value) in samples {
            ...
            let row = readings
                .entry(instance.pid)
                .or_default()
                .entry((instance.adapter.clone(), engine.clone()))
                .or_insert_with(|| GpuReading {
    ...
    for row in &mut after {
        if before
            .iter()
            .any(|p| policy::same_process(&p.identity, &row.identity) && p.identity.created != 0)
        {
            if let Some(values) = readings.remove(&row.identity.pid) {
                row.gpu = values.into_values().collect();
            }
        }
    }
    after.sort_by(|a, b| {
    ...
    let status = if notices.is_empty() {
        ...
        format!("Some GPU measurements unavailable: {}", notices.join(" | "))
    };
    Ok(Snapshot {
        processes: after,
        gpu_status: status,
