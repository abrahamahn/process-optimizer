#![cfg(windows)]
use process_optimizer::{engine::{self, Backend}, journal::{Database, Journal}, model::*, windows::process::{self, WindowsBackend}};
use std::{path::Path, process::{Child, Command}, thread, time::{Duration, Instant}};
...
impl Fixture {
    fn start(window: bool) -> Self {
        ...
        if window { c.arg("--window"); }
        Self(c.creation_flags(0x08000000).spawn().expect("spawn test-owned fixture"))
    }
    fn identity(&self) -> Identity {
        let deadline = Instant::now() + Duration::from_secs(5);
        loop {
            if let Ok(id) = process::identity(self.0.id()) { return id; }
...
fn session(target: Identity, action: ActionKind) -> Session {
    Session::new(format!("test-{}", target.pid), Plan { game_path: r"C:\TestOnly\NotAnActualGame.exe".into(), game: None, actions: vec![ApprovedAction { target, action }], protected_paths: vec![], options: Options { gpu_priority: false, cpu_priority: true, close_timeout_ms: 1000, ..Options::default() }, consent: true, force_consent: action == ActionKind::ForceClose })
}
...
fn priority_round_trip_on_our_own_fixture_only() {
    ...
    assert_eq!(backend.read(&id, Property::CpuPriority).unwrap(), original.background());
...
fn incorrect_creation_time_is_rejected_before_any_write() {
    let fixture = Fixture::start(false); let mut id = fixture.identity(); id.created += 1;
    let mut backend = WindowsBackend::new(None, vec![], None).unwrap();
...
fn graceful_request_does_not_kill_windowless_fixture() {
    let fixture = Fixture::start(false); let id = fixture.identity();
    let mut backend = WindowsBackend::new(None, vec![], None).unwrap();
    assert_eq!(backend.close(&id, false, 1000).unwrap(), CloseState::KeptOpen);
    assert!(process::alive(&id).unwrap());
}
...
fn wm_close_reaches_our_test_window() {
    let fixture = Fixture::start(true); let id = fixture.identity();
    let mut backend = WindowsBackend::new(None, vec![], None).unwrap();
    let deadline = Instant::now() + Duration::from_secs(8);
    loop {
        match backend.close(&id, false, 1000) {
            Ok(CloseState::ClosedGracefully) => break,
            _ => { assert!(Instant::now() < deadline, "test window did not accept WM_CLOSE"); thread::sleep(Duration::from_millis(100)); }
        }
    }
}
...
fn sqlite_durable_record_can_be_reopened() {
    let path = std::env::temp_dir().join(format!("optimizer-journal-test-{}-{}.sqlite3", std::process::id(), process_optimizer::windows::runner::fresh_id()));
    let fixture = Fixture::start(false); let mut s = session(fixture.identity(), ActionKind::LowerPriorities);
    { let mut db = Database::open(&path).unwrap(); db.create(&s).unwrap(); s.stage = Stage::RecoveryNeeded; db.save(&s).unwrap(); }
    { let db = Database::open(&path).unwrap(); assert_eq!(db.active().unwrap().unwrap().stage, Stage::RecoveryNeeded); }
